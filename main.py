...
from task_trajectory import task_trajectory, TRAJ_GARAGE
from task_master     import task_master

# Constants
_DEG_TO_RAD = 3.14159 / 180.0
_PSI_SIGN   = -1  # flip if IMU heading increases opposite to positive rotation
...
leftSetPoint  = Share("f", name="Left SP");  leftSetPoint.put(200.0)
rightSetPoint = Share("f", name="Right SP"); rightSetPoint.put(200.0)
...
lineKp  = Share("f", name="Line Kp");  lineKp.put(40.0)
lineKi  = Share("f", name="Line Ki");  lineKi.put(0.0)
lineKd  = Share("f", name="Line Kd");  lineKd.put(2.0)
...
baseSpeed          = Share("f", name="Base Speed");          baseSpeed.put(100.0)
...
# ── NEW: line sensor shares for master task ───────────────────────────────────
...
lineSensorTask = task_line(
    ...
    centroid_share = centroid_share,    # ← NEW
    line_detected_share = line_detected # ← NEW
)
...
bumpTask = task_crash(
    crash_detect, bump_pins
)

trajectoryTask = task_trajectory(
    x_pos_hat     = x_pos_hat,
    y_pos_hat     = y_pos_hat,
    psi_hat       = psi_hat,
    omL_hat       = omL_hat,
...
masterTask = task_master(
    ...
    estimatorGo    = estimatorGo,   # Bug 4 fix: master owns estimator lifecycle
)
...
task_list.append(Task(trajectoryTask.run, name="Trajectory",  priority=2, period=50,  profile=False))
